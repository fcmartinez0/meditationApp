...
import type { PieceRating, PieceSpec, Section } from './types';
...
export function nextSpec(section: Section, ratings: PieceRating[]): PieceSpec {
  ...
  // Pick the scale with the best net score (likes minus dislikes), exploring rarely.
  ...
  let bestScale = pick(SCALES) as string;
  let best = -Infinity;
  ...
  // Favor binaural settings that appear among liked pieces.
